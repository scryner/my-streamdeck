...
func (d *Device) resetSignalsLocked() {
	d.extra.disconnect = false
	d.extra.disconnectCh = make(chan bool)
	d.extra.disconnectSignaled = false
	d.extra.closeCh = make(chan struct{})
	d.extra.closeDoneCh = make(chan struct{})
	d.extra.closeStarted = false
	d.extra.closeSignaled = false
...
func (d *Device) signalCloseLocked() {
	if d.extra.closeSignaled || d.extra.closeCh == nil {
		return
	}
	close(d.extra.closeCh)
	d.extra.closeSignaled = true
}

func (d *Device) signalDisconnectLocked() {
	d.extra.disconnect = true
	if d.extra.disconnectSignaled || d.extra.disconnectCh == nil {
		return
	}
	close(d.extra.disconnectCh)
	d.extra.disconnectSignaled = true
}
...
func (d *Device) close() error {
	...
	if d.extra.closeStarted {
		doneCh := d.extra.closeDoneCh
		d.extra.mtx.Unlock()
		if doneCh != nil {
			<-doneCh
		}
	...
	d.extra.closeStarted = true
	d.signalCloseLocked()

	file := d.extra.file
	options := d.extra.options
	runloop := d.extra.runloop
	disconnect := d.extra.disconnect
	inputClosed := d.extra.inputClosed
	...
	if !disconnect {
		if len(inputBuffer) > 0 {
			_IOHIDDeviceRegisterInputReportCallback(file, unsafe.Pointer(&inputBuffer[0]), _CFIndex(d.reportInputLength+1), 0, nil)
		}
		_IOHIDDeviceRegisterRemovalCallback(file, 0, nil)
		if runloop != 0 {
			_IOHIDDeviceUnscheduleFromRunLoop(file, runloop, **(**_CFStringRef)(unsafe.Pointer(&_kCFRunLoopDefaultMode)))
		}
	}

	if !inputClosed && runloop != 0 {
